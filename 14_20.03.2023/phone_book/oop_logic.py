import uuid  #позволяет создать уникальный id
...
class PhoneBookEntry:
    ...
    def remove_number(self,numbers):
        for number in numbers:
            i=self.__numbers.index(number)#находим индекс от номера
            if i>=0:
    ...
    def __str__(self):
        #если заменить название на __str__ то в конце можно вывести просто entry так как все обекты в PY изначально в стр
